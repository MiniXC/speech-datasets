    ...
    def __call__(self, batch):
        ...
        for i in range(len(new_batch)):
            ...
            try:
                ...
                p_energy = signal.convolve(p_energy, np.ones(5) / 5, mode="same")
                tres = 0.01
                    ...
                    if p:
                        ...
                        if silence_started:
                            ...
                            if j >= len(p_energy) - 5:
                                j = len(p_energy) - 1
                            silence_end = j + 1
                            silences_indicies_as_tuples.append(
                ...
                for silence_start, silence_end in merged_silences:
                    if (
                        silence_end - silence_start < 25
                        and silence_end - silence_start > 5
                    ):
                        predicted_ids[i][silence_start:silence_end] = (
                            len(self.id2phone) - 1
                        )
                ...
                vad = (vad - self.vad_range[0]) / (
                    self.vad_range[1] - self.vad_range[0]
                )
                ...
                for j, d in enumerate(phone_lengths):
                    ...
                    prosody[
                        j, vals_per_window + 1 : vals_per_window * 2 + 1
                    ] = resample(energy_window, vals_per_window)
                    prosody[
                ...
                mel = (mel - self.mel_range[0]) / (
                    self.mel_range[1] - self.mel_range[0]
                )
                mel = to_img(mel, min_val=0, max_val=1)
                mel = Image.fromarray(mel.numpy().T)
                mel.save(save_dir / f"{utt_hash}_mel.png")
            except:
                # delete files possibly created (text, phones, prosody, speaker, mel)
                silent_remove(save_dir / f"{utt_hash}_text.txt")
                silent_remove(save_dir / f"{utt_hash}_phones.npy")
                silent_remove(save_dir / f"{utt_hash}_prosody.png")
                silent_remove(save_dir / f"{utt_hash}_speaker.png")
                silent_remove(save_dir / f"{utt_hash}_mel.png")
                silent_remove(save_dir / f"{utt_hash}_speaker.npy")
                print(f"Error processing {utt_hash}")
                continue
